...
Handlebars.registerHelper('ifEquals', function(arg1, arg2, options) {
  ...
  let hidden = false;
  if(arg2.length > 0) {
    arg2.forEach(function(element){
      if(element.toString() === arg1.toString()) {
        hidden = true;
      }
    });
  }
  return hidden === true ? options.inverse(thisObject) : options.fn(thisObject);
});
...
Handlebars.registerPartial('button', templates.button);

Handlebars.registerHelper('ifUrl', function(input, options){
  return input.toString().startsWith('http') ? options.fn(this) : options.inverse(this);
});